...
public class MySinglyLinkedList<E> implements MyListInterface<E> {
	...
	*/
	...
	public void mixin(MySinglyLinkedList<E> otherList) {
		if(size()==0) {
			head = otherList.getHeaderNode();
		}
		
		/*
		 * for (Node<E> x = head; x != null; x = x.next) { queue.offer(x); }
		 */
		Node<E> node = head;
		Node<E> othnode = otherList.getHeaderNode();
